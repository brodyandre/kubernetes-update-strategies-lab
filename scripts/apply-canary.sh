...
log_info "Applying stable Blue deployment..."
kubectl apply -f "${ROOT_DIR}/manifests/04-canary/deployment-stable-blue.yaml"

log_info "Applying Green canary deployment..."
kubectl apply -f "${ROOT_DIR}/manifests/04-canary/deployment-canary-green.yaml"

log_info "Applying Canary service..."
kubectl apply -f "${ROOT_DIR}/manifests/04-canary/service.yaml"

log_info "Waiting for rollout of deployment 'canary-stable-blue'..."
kubectl rollout status deployment/canary-stable-blue -n "${NAMESPACE}" --timeout=120s

log_info "Waiting for rollout of deployment 'canary-green'..."
kubectl rollout status deployment/canary-green -n "${NAMESPACE}" --timeout=120s

log_ok "Canary strategy applied successfully with approximately 80% blue and 20% green."
show_resource_summary
