...
log_info "Applying Ramped service manifest..."
kubectl apply -f "${ROOT_DIR}/manifests/02-ramped/service.yaml"

log_info "Applying Ramped deployment with blue version..."
kubectl apply -f "${ROOT_DIR}/manifests/02-ramped/deployment-blue.yaml"

log_info "Waiting for rollout of deployment 'ramped-demo'..."
kubectl rollout status deployment/ramped-demo -n "${NAMESPACE}" --timeout=120s

log_ok "Ramped strategy applied successfully with the blue version."
show_resource_summary

echo
log_info "Next step to update to green:"
echo "kubectl apply -f manifests/02-ramped/deployment-green.yaml"
